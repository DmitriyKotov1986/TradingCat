...
PKLinesList BingxKLine::parseKLine(const QByteArray &answer)
{
    ...
    try
    {
        ...
        for (const auto& klineData: data)
        {
            const auto kline = JSONReadMap(klineData, "Root/data/[]");

            const auto openDateTime = QDateTime::fromMSecsSinceEpoch(JSONReadMapNumber<qint64>(kline, "time", "Root/data/[]/time", 0).value_or(0));
